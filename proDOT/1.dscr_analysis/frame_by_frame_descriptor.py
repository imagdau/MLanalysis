...
def ds_frame_by_frame(sys_name, ds_obj, 
    ...
    for f in tqdm(range_):
        snap_tmp = read(sys_name, index=str(f-1)+':'+str(f), format='xyz')
        ps_tmp = ds_obj.calc_descriptor(snap_tmp)
        ps_list.append(ps_tmp[0])
